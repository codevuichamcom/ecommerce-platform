...
| Current sprint    | **Day 24 ✅ Done** — SQL vs NoSQL vs ES comparative deep-dive (decision matrix). Day doc-only (0 dòng service code mới — đúng kế hoạch "day của cái đầu"). **Decision matrix 8 use case × 4 storage** (Postgres/Redis/Mongo/ES) verdict ✅/🟡/❌ + reasoning + **ngưỡng đảo chiều** mỗi ô. **5-axis** so sánh (consistency/schema/query/scaling/ops cost). **PACELC** mapping: Postgres PC/EC · Mongo PC/EL · ES PA/EL · Redis PC/EL (vế ELC = chỗ phân biệt senior, "Mongo CP hay AP" là câu thiếu vế). **3 anti-pattern** drill: Mongo-cho-invariant-chặt · Postgres-EAV · ES-làm-primary. `system-overview.md` diagram tô màu 4 storage paradigm + classDef, mọi derived store (ES/Mongo/Redis) sync từ Postgres = source of truth. Persona NexaShop/Anh Khải (Principal Architect) hỏi "sao không Mongo luôn cho attributes" → defend JSONB (query đơn giản + 1 source of truth + tránh dual-write). Evolution ch.24 "Bốn món đồ nghề và gã chỉ thích cầm búa" (law of instrument: cờ-lê/dao bấm/băng keo/kính lúp). Previous: **Day 23 ✅ Done** — MongoDB 7 (event store + flexible attributes) |
| Next up           | **Day 25 — Polyglot persistence review + anti-pattern (data ownership map) — cuối Week 4, có CV bullet** |
| Sprints completed | 24 / 40                                     |
...
| Docs created      | 126 (added 5 Day 24: lesson 24 + 24b + issue 24 + interview day-24 + evolution ch.24; + update system-overview diagram) |
