...
| Current sprint    | **Day 16 ✅ Done** — Slow query tuning. V5 migration: `pg_trgm` extension + GIN trigram trên `LOWER(name)` cho substring search → planner Seq Scan (1M rows, 2.5s) → Bitmap Index Scan (~45ms, 57× faster). Covering index `(category_id, created_at DESC) INCLUDE (id, name, price, status) WHERE status='ACTIVE'` → Index-Only Scan `Heap Fetches: 0`, sub-millisecond list-by-category. Seed script reproducible 1M products (outside Flyway). DebugExplainController gated `app.debug.explain.enabled=true` cho demo EXPLAIN before/after. 4 doc cross-link: performance/16 EXPLAIN breakdown + lesson/16 5-loại-index decision matrix + issue/16 9-section 4-approach + interview/16 5 Q&A. Evolution chapter 16 "Kính hiển vi". Trade-off chốt: GIN index 187 MB, write +50%, accepted vì read:write 100:1. ES postpone Day 22 đúng lý do business (relevance + faceting + scale). Previous: **Day 15 ✅ Done** — 2-tier cache (Caffeine L1 60s + Redis L2 5min) + XFetch stampede protection, P99 250ms → 18ms, DB load -98% |
| Next up           | **Day 17 — JPA N+1 (EntityGraph, JOIN FETCH, projection DTO)** |
| Sprints completed | 16 / 40                                     |
