...
| Current sprint    | **Day 20 ✅ Done** — Load testing. k6 **open model** (`ramping-arrival-rate`, chống coordinated omission) cho place-order + browse-products; threshold gate P95<200/P99<500/err<1% (exit≠0 = CI fail). Observability stack `docker-compose.observability.yml`: Prometheus (5s scrape `/actuator/prometheus`) + **Tempo** (Zipkin receiver cổng 9412 → zero Java dep change) + Grafana dashboard `load-test-overview` provisioned. order-service thêm `micrometer-registry-prometheus` + `percentiles-histogram` bucket + profile `platform` (VT off, Tomcat 200) đối chứng. **Phát hiện chính**: bật VT mà P99 place-order nổ 120ms→2.1s, CPU chỉ 35%, Hikari pending 150+ → bottleneck DỜI sang connection pool (issue 20); Tempo trace cho thấy `Connection Acquisition` ăn 1.8s/2.1s, DB thật 4ms. Fix = Little's Law size pool 30 + connection-timeout 2s (không tăng mù 500). VT vs platform: read VT thắng rõ (nuốt 1000+ concurrent), write hoà (cùng nghẽn pool). 6 doc + evolution ch.20 "Ông huấn luyện viên tàn nhẫn". Build xanh, order-service test pass. Previous: **Day 19 ✅ Done** — concurrency, JMH lock bench + VT pinning JFR + fencing-token distributed lock |
| Next up           | **Day 21 — Review performance + interview round (Week 3 mock)** |
| Sprints completed | 20 / 40                                     |
