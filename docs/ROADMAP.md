...
| Current sprint    | **Day 19 ✅ Done** — Java concurrency. Module mới `concurrency-lab` (JMH + `--enable-preview` cô lập). JMH 8-thread read-mostly: `synchronized` 7.3K / `ReentrantLock` 20.8K / `StampedLock` optimistic 5.6M ops/ms (~770× — optimistic không acquire lock nên không cache-line bouncing; phải copy field trước `validate`). VT vs platform: VT ≈ ioMillis, platform-200 ≈ ×50 cho IO-bound (KHÔNG overclaim — CPU-bound VT không nhanh hơn). **Pinning** chứng minh bằng JFR `jdk.VirtualThreadPinned`: `synchronized`+block ~200 pin vs `ReentrantLock` 0 → fix bằng đổi lock. Structured concurrency `StructuredTaskScope.ShutdownOnFailure` fan-out cart+product+inventory fail-fast (2 test pass). Distributed lock: `RedisDistributedLock` (SET NX PX + Lua owner-check release + **fencing token** INCR) vào common-lib auto-config; dùng thật ở `InventorySnapshotJob` (leader-elect) + DB fence guard `ON CONFLICT WHERE last_fencing_token < EXCLUDED` chặn GC-pause split-brain (issue 19). Build xanh, StockTest 9 pass, lock IT 4 gated skip. 5 doc + evolution ch.19 "Anh bảo vệ phòng VIP một chìa". Previous: **Day 18 ✅ Done** — keyset pagination, deep offset 2.4s → 3ms |
| Next up           | **Day 20 — Load testing (k6 + Grafana + OTel trace timeline)** |
| Sprints completed | 19 / 40                                     |
