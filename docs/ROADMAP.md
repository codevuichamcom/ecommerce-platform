...
| Current sprint    | **Day 15 ✅ Done** — 2-tier cache (Caffeine L1 60s + Redis L2 5min) cho `getProduct/getBySlug` qua Spring `@Cacheable`/`@CacheEvict`; XFetch probabilistic early expiration chống stampede (test 100 concurrent → loader call <25); Micrometer hit/miss/xfetch metrics qua `/actuator/prometheus`; ADR-008 + 7 docs cross-link; expected P99 250ms → 18ms, DB load -98%. Plan: pub/sub L1 invalidation + hot-key sharding postpone Day 20+. Previous: **Day 14 ✅ Done** (Week 2 wrap — mock interview 10 câu Kafka senior 5 fundamentals + 5 production scenario, self-grade 9 strong / 1 borderline (trace E2E qua outbox path chưa verify Zipkin thật) / 0 fail; review brutally honest Week 2 code 9 finding 🔴 3 (inventory consumer swallow RuntimeException + partial-success không atomic + chưa idempotent) + 🟡 4 (OutboxRelay sequential publish bottleneck · TRUSTED_PACKAGES=* · observation-enabled chưa explicit · notification dedup release race sau side effect) + 🟢 2 (schema registry gap + outbox không leader election); CV bullet Week 2 2 bullet metric-driven + elevator pitch v2 90s; evolution chapter 14 "Tấm gương soi"; 2 entry mới [05] catch-all RuntimeException + [06] dedup release sau side effect vào ai-junior-traps cumulative checklist. KHÔNG code mới — freeze trước campaign 6/6) |
| Next up           | **Day 16 — Slow query tuning (EXPLAIN ANALYZE, indexes)** |
| Sprints completed | 15 / 40                                     |
