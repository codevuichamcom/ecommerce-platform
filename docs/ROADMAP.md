...
| Current sprint    | **Day 17 ✅ Done** — JPA N+1. Endpoint mới `GET /orders` (list "Đơn hàng của tôi") phơi bày N+1: `Order.items` EAGER từ Day 6 → list N đơn = 1+N query (3.2s, 41 query đo bằng Hibernate `Statistics`). Fix 4 nấc thang ở `OrderRepository`: derived EAGER (N+1) → `@EntityGraph` (1 query nhưng `HHH000104` in-memory pagination) → `JOIN FETCH` (1 query, không phân trang bag, `MultipleBagFetchException` nếu ≥2 bag) → **projection DTO** (`OrderSummaryView` constructor expression + `size(items)` subquery COUNT, ≤2 query, pagination ở DB) — chosen cho list path (CQRS-lite: read model tách write model). `listMyOrders()` sort whitelist + size cap 100, scope theo userId. DebugController `/debug/orders/n-plus-one` + NPlusOneDemoService gated chạy 4 nấc side-by-side. IT gated assert cả 2 chiều (projection ≤2, nấc-0 ≥1+N). 41 query → 2 query. 4 doc cross-link + evolution ch.17 "Anh bồi bàn chạy bộ". Previous: **Day 16 ✅ Done** — Slow query tuning, GIN trigram + covering index, p95 2.5s → 45ms (57×) |
| Next up           | **Day 18 — Pagination at scale (offset → keyset/seek)** |
| Sprints completed | 17 / 40                                     |
