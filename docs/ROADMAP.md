...
| Current sprint    | **Day 22 ✅ Done** — Elasticsearch 8 product search (Week 4 data layer mở màn). Inverted index + BM25 relevance (`name^3`) + fuzzy AUTO ("iphon"→iPhone) + faceted aggregation (brand/category) + highlight. Sync Postgres→ES app-level qua Kafka `product.upserted`/`product.deleted` (CDC-lite, key=productId ordering, publish afterCommit). Postgres GIỮ source of truth; ES = derived index + graceful fallback GIN khi ES down (`X-Search-Source` header). Drift handling: `/admin/search/{reindex,drift}` reconcile. 5 integration test PASS trên ES 8.15 container thật (fuzzy/relevance/facet/highlight/brand-filter). Trap [07]: AI nhớ nhầm ES client API 7.x vs 8.15 tagged-union RangeQuery. Evolution ch.22 "Ông thầy bói đọc vị" (ES = thầy bói đọc bản phô-tô, Postgres = sổ gốc). Previous: **Day 21 ✅ Done** — Review performance Week 3 (23 findings) + mock 10 Q (9 strong) + CV bullet 4× latency / 10× throughput |
| Next up           | **Day 23 — MongoDB integration (event store + flexible attributes)** |
| Sprints completed | 22 / 40                                     |
| Services built    | 7 / 9 (`common-lib` ✅, `auth-service` ✅, `product-service` ✅ (+ES search), `inventory-service` ✅, `cart-service` ✅, `order-service` ✅ (outbox), `payment-service` ✅, `notification-service` ✅, gateway/analytics ⏳) |
| Docs created      | 114 (added 6 Day 22: lesson 22 + 22b + performance 22 + issue 22 + interview day-22 + ADR-010; + evolution ch.22 + trap [07]) |
