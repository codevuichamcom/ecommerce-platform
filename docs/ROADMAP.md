...
| Current sprint    | **Day 13 ✅ Done** (Transactional outbox trả dual-write debt Day 9: `outbox_event` table partial index PENDING + CHECK status whitelist; `OutboxEvent` JPA entity với lifecycle markSent/recordFailure/shouldGiveUp; `OutboxRecorder` ghi cùng tx business write — Jackson serialize fail-fast; `OutboxRelay` `@Scheduled(fixedDelay=1s)` + `FOR UPDATE SKIP LOCKED` (Hibernate lock.timeout=-2) multi-instance race-free + `@Transactional(REQUIRES_NEW)` per-event tx isolation + JsonNode raw publish tránh double-quote; `PlaceOrderUseCase` refactor bỏ direct kafkaTemplate.send → atomic DB-only; ADR-009 5 alternatives compared (chosen outbox poll over Debezium CDC do DBA chưa enable wal_level=logical); 8 unit test PASS — total 24 xanh) |
| Next up           | **Day 14 — Mock interview Week 2 (Kafka senior questions + review)** |
| Sprints completed | 13 / 40                                     |
