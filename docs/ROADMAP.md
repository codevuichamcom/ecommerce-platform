...
| Current sprint    | **Day 23 ✅ Done** — MongoDB 7 integration (event store + flexible attributes, dùng CÓ CHỦ Ý). `analytics-service` (service #8) Mongo event store: event đa hình (`product_viewed`/`cart_updated`/`order_placed`) 2 nguồn (Kafka `order.created` + HTTP beacon `/analytics/track`); aggregation pipeline top-products + conversion funnel; compound `(type,occurredAt)` + TTL 90d index tường minh. `product-service` catalog read-model: flexible attributes embed (TV vs áo khác shape), sync qua **cùng event `product.upserted`** fan-out (ES `-indexer` + Mongo `-catalog`), filter động `attributes.<key>`. Postgres VẪN source of truth (anti-cargo-cult). 7 IT PASS trên Mongo 7.0 container thật (4 analytics + 3 catalog). Trap [08]: common-lib `GlobalExceptionHandler` hard-couple spring-security → tách `SecurityExceptionHandler` `@ConditionalOnClass`. Evolution ch.23 "Cuốn sổ không dòng kẻ" (Mongo = bác thư ký chép sổ trắng, đừng nhờ giữ 2 trang khớp = no multi-doc txn). Previous: **Day 22 ✅ Done** — Elasticsearch 8 product search (inverted index + BM25 + fuzzy + facet + sync Postgres→ES) |
| Next up           | **Day 24 — SQL vs NoSQL vs ES comparative deep-dive (decision matrix)** |
| Sprints completed | 23 / 40                                     |
...
| Docs created      | 121 (added 7 Day 23: lesson 23 + 23b + issue 23 + interview day-23 + ADR-011 + evolution ch.23 + trap [08]) |
